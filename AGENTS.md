...
`dmd` on the phone owns `/dev/umts_dm0`. Host SCAT never opens that node. Live host decode is `scripts/live_tail_ring.sh` until Ctrl-C.

SCAT `-a` is USB bus:address; GSMTAP host is `-H`. A dump filename must contain `.sdm` so SCAT uses the logger parser (`run_logger`), not raw SDM.
