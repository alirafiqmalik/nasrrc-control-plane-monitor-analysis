#!/usr/bin/env bash
# Ring-rotation follower proof. No device: a fake `adb` and a fake SCAT stand in.
set -uo pipefail
...
wait_for() { # marker file timeout_deciseconds
  local i=0
  while (( i < $3 )); do grep -q "$1" "$2" 2>/dev/null && return 0; sleep 0.1; i=$((i+1)); done
  return 1
}
fail() { echo "FAIL: $1" >&2; echo "--- script log ---" >&2; cat "$TMP/live.log" >&2; exit 1; }

PATH="$TMP/bin:$PATH" SCAT_PY="$TMP/bin/fakepy" FIFO="$TMP/live.sdm" \
...
echo "PASS: followed the ring rotation, reaped by a quote-safe pattern, restored logging"
