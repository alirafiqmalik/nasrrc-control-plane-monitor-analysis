...
`dmd` owns `/dev/umts_dm0` on the phone, but not exclusively — the node serves the same live stream to a second reader (`scripts/experiment_scat_diag.sh node`, on trial). The stable path reads `sbuff_*.sdm` over adb. GSMTAP host is `-H` (not `-a`). Dump names must contain `.sdm`. Raw `.sdm`/`.pcap` are gitignored (subscriber identities). `baseband-attach/` is the frozen 2026-08-13 capture. `fixtures/synthetic-lte/` is a hand-encoded GSMTAP pcap with no subscriber data, so the analyzer has something to test against.
